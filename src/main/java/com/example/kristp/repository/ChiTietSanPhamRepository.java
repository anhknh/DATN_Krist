...
public interface ChiTietSanPhamRepository extends JpaRepository<ChiTietSanPham, Integer> {
    ...
    ChiTietSanPham findByQrCode(String qrCode);
        ...
        @Query("SELECT DISTINCT ms FROM ChiTietSanPham ctp " +
                "JOIN ctp.sanPham sp " +
                "JOIN ctp.mau ms " +
                "WHERE sp.id = :sanPhamId and ctp.trangThai = 1 and ctp.soLuong > 0")
        ...
        @Query("SELECT ctp FROM ChiTietSanPham ctp " +
                ...
                "JOIN ctp.mau ms " +
                "JOIN ctp.size s " +
                "WHERE sp.id = :sanPhamId AND ms.id = :mauSacId AND s.id = :sizeId and ctp.trangThai = 1 and ctp.soLuong > 0")
...
//Lấy các chi tiết sản phẩm san pham có trạng thái là đang hoạt động
        @Query("from ChiTietSanPham ctsp join SanPham sp on ctsp.sanPham.id = sp.id where ctsp.trangThai = 1 and ctsp.soLuong >= 0")
    List<ChiTietSanPham> findAllProductDetailsSPHD();

        @Query(value = "select id_size from chi_tiet_san_pham where id_san_pham = :id and id_mau_sac = :idMau and trang_thai = 1 and so_luong > 0", nativeQuery = true)
